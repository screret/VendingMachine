...
public class VenderBlockPriceScreen extends AbstractContainerScreen<VenderPriceEditorMenu> {
    ...
    public void init(){
        ...
        mainPageButton = new VenderTabButton(leftPos + this.imageWidth, topPos + 2, 32, 28, new TranslatableComponent("gui.vendingmachine.button_tab_main"), onTabButtonPress(true), true, false);
        thisPageButton = new VenderTabButton(leftPos + this.imageWidth, topPos + 30, 32, 28, new TranslatableComponent("gui.vendingmachine.button_tab_price"), onTabButtonPress(false), false, true);
    ...
    private static final Component addPriceGuiName = new TranslatableComponent("gui.vendingmachine.button_add_price");
    ...
    private void renderButtons(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
        if(renderPriceMenu){
            ...
            if(!itemPriceInput.getValue().matches("\\u0000+|\\ufeff+|\\u200b+|\\u200c+|\\u200d+| +|^$"))
    ...
    private static final Component toolTipLeftClickAddPrice = new TranslatableComponent("tooltip.vendingmachine.left_click_price");
    private static final Component toolTipRightClickRemovePrice = new TranslatableComponent("tooltip.vendingmachine.right_click_price");
    ...
    protected void renderTooltip(PoseStack poseStack, ItemStack itemStack, int mouseX, int mouseY) {
        if(!renderPriceMenu){
            ...
            tooltip.add(1, new TranslatableComponent("tooltip.vendingmachine.price", Util.DECIMAL_FORMAT.format(price), VendingMachineConfig.GENERAL.isStackPrices.get() ? itemStack.getMaxStackSize() : 1));
        ...
        if(slot != null && (itemPriceInput == null || !itemPriceInput.isActive()) && slot.getSlotIndex() < VenderBlockMenu.LAST_CONTAINER_SLOT_INDEX){
            ...
            if(selectedItem != ItemStack.EMPTY){
                if(mouseButton == 0){
                    ...
                    String selectedItemValueString = val == null ? "" : Util.DECIMAL_FORMAT.format(val);
                    createAddMenu(selectedItemValueString);
                }else if(mouseButton == 1){
                    VendingMachineBlockEntity tile = menu.getTile();
                    VendingMachine.NETWORK_HANDLER.sendToServer(new PacketPriceChangeC2S(tile.getBlockPos(), selectedItem, currentPrice, false));
    ...
    public void createAddMenu(String inputVal){
        ...
        if(itemPriceInput == null){
            itemPriceInput = new EditBox(this.minecraft.font, leftPos + 32, topPos + 64, 64, 16, TextComponent.EMPTY);
        ...
        if(addPriceMenuButtonContinue == null){
            addPriceMenuButtonContinue = new Button(leftPos + 32, topPos + 84, 32, 16, new TranslatableComponent("gui.vendingmachine.button_continue"), onAddedPress);
        ...
        if(addPriceMenuButtonCancel == null){
            addPriceMenuButtonCancel = new Button(leftPos + 72, topPos + 84, 32, 16, new TranslatableComponent("gui.vendingmachine.button_cancel"), hideAddMenu);
    ...
    public Button.OnPress onTabButtonPress(boolean isMain){
        return new Button.OnPress() {
            ...
            public void onPress(Button button) {
                VendingMachineBlockEntity tile = menu.getTile();
                if (isMain) {
                    VendingMachine.NETWORK_HANDLER.sendToServer(new OpenGuiPacketC2S(tile.getBlockPos(), true));
    ...
    public Button.OnPress onAddedPress = new Button.OnPress() {
        ...
        public void onPress(Button button) {
            if(menu.selectedSlot != null){
                currentPrice = itemPriceInput.getValue().equals("") ? 0 : Float.parseFloat(itemPriceInput.getValue());
                VendingMachineBlockEntity tile = menu.getTile();
                VendingMachine.NETWORK_HANDLER.sendToServer(new PacketPriceChangeC2S(tile.getBlockPos(), menu.selectedSlot.getItem(), currentPrice, true));
                removeAddMenu();
