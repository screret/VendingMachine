...
public class MoneyConversionRecipeProvider extends RecipeProvider {
    ...
    protected static void conversion(Consumer<FinishedRecipe> recipe, float resultMoneyValue, int resultCount, float ingredientMoneyValue, int ingredientCount, String id) {
        MoneyConversionRecipeBuilder.conversion(Util.setMoneyValue(new ItemStack(Registration.MONEY.get(), resultCount), resultMoneyValue)).requires(Util.setMoneyValue(new ItemStack(Registration.MONEY.get(), ingredientCount), ingredientMoneyValue)).group("money_conversion").unlockedBy("has_money", has(Registration.MONEY.get())).save(recipe, new ResourceLocation(VendingMachine.MODID, id));
    }
}
